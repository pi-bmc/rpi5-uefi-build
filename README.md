...
line), `config.txt`, the `bcm2712*.dtb` device trees (built from kernel.org
stable, see `linux-stable-dtbs`: flat and under `broadcom/` for the VPU
bootloader, and under `dtb/<version>/` -- one directory per 6.18.y release
that changed them -- for FdtDxe's nearest-older-version lookup) and
`overlays/` (pinned raspberrypi/firmware release, see `rpi-boot-dtbs`).
Flash it with
...
| `meta-rpi5-uefi/recipes-bsp/linux-stable-dtbs/linux-stable-dtbs_6.18.bb` | kernel.org `linux-6.18.tar.xz` + cumulative `patch-6.18.N` diffs, checksum-pinned; only the device-tree subset is ever unpacked | `bcm2712*.dtb` with board overlays baked in, built at every 6.18.y release that changed them, under `by-version/<version>/` (for FdtDxe's nearest-older-version lookup, edk2-platforms patch 0038) plus the newest base tree flat (for the VPU bootloader) |
