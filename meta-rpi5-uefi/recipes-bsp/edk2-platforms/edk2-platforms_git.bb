...
DESCRIPTION = "The edk2-platforms source tree that rpi5-uefi-firmware builds \
               ...
               firmware build so a 96 MB fetch and a 20-odd-patch series are \
...
# two pieces here: files/edk2-platforms/ (the port's ADDED files, copied in
# after unpack) plus 0000-edk2-platforms-RPi5-port.patch (its changes to
# files that exist upstream, 43 of them). As imported, the two reconstructed
# the fork byte-identical within every package tree the RPi5 build compiles;
# since then, this layer's own changes to overlay-carried files are edited
# directly in files/edk2-platforms/ (their history is this repo's git log),
# so the overlay has deliberately diverged from the fork where this layer
# changed it. The fork's bulk-sync churn in other vendors' trees (187 files
...
#                                       Rpi5FmpDeviceLib
#
# RPi5.dsc and RPi5.fdf (also overlay files) list every one of these directly
# and unconditionally, exactly as RPi4.dsc/.fdf list theirs, and the .dec
# content the retired packages carried was folded into RpiSiliconPkg.dec and
# RPi5.dec. The firmware recipe therefore inserts nothing for any of them.
#
...
#
# The split rule: a change to a file the overlay itself carries is made
# directly in files/edk2-platforms/, never as a patch; the patch series
# exists only for files that come from the upstream checkout. Patch order
# matters and follows SRC_URI order: 0000 (the fork's changed files) first,
# then this layer's own fixes on top. Numbering is sparse: numbers whose
# patches were folded into the overlay (0001, 0003, 0011/0012, 0014, 0016,
# 0018) stay retired, as do abandoned experiments noted below.
#
...
#   brings none. Paired with the Device-Tree-by-default SystemTableMode
#   setting in the overlay's RPi5.dsc: in ACPI mode FdtDxe never runs at all.
#
