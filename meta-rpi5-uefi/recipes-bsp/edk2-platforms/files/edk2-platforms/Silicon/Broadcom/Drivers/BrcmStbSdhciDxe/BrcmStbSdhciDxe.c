  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a: Failed to get protocol. Status=%r\n",
      __func__,
      Status
      ));
    return EFI_UNSUPPORTED;
  ...
  )
{
  ...
  //
  MmioAndThenOr32 (
    This->CfgAddress + SDIO_CFG_MAX_50MHZ_MODE,
  ...
  if (This->IsSlotRemovable) {
    MmioAndThenOr32 (
      This->CfgAddress + SDIO_CFG_SD_PIN_SEL,
      ~SDIO_CFG_SD_PIN_SEL_MASK,
      SDIO_CFG_SD_PIN_SEL_CARD
      );
  } else {
    MmioAndThenOr32 (
      This->CfgAddress + SDIO_CFG_CTRL,
  ...
  while (TRUE) {
    ...
    if (EFI_ERROR (Status)) {
      if (Status != EFI_NOT_FOUND) {
        DEBUG ((
          DEBUG_ERROR,
          "%a: Failed to locate protocol. Status=%r\n",
          __func__,
          Status
          ));
      }
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_ERROR,
        "%a: Failed to start device. Status=%r\n",
        __func__,
        Status
        ));
      break;
