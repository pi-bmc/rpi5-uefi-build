  ...
  )
{
  ...
  if (ConfigLang == NULL) {
    ...
    if (!EFI_ERROR (Status) || (Status == EFI_NOT_FOUND)) {
      ...
      //
      // One-element list, so the entry goes in slot 0; the parsed {N}
      // instance travels in .Index. (The upstream template wrote
      // List[Index], a heap overwrite for any instance other than 0.)
      //
